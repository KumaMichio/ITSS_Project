...
//        return orderRepository.save(order);
    ...
    public List<OrderDTO> getOrdersByUserId(int userId) {
        List<Order> orders = orderRepository.findAll().stream().filter((Order order) -> order.getUser().getId() == userId).sorted(Comparator.comparing(Order::getOrderId).reversed()).toList();
