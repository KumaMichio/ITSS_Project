...
public class EmailService {
    ...
    private String generateOrderConfirmationEmailContent(Order order) {
        ...
        content.append("Kính chào ").append(order.getDeliveryInformation().getName()).append(",\n\n");
        ...
        content.append("Phương thức giao hàng: ").append(order.getShippingMethod().getMethod_name()).append("\n\n");

        content.append("THÔNG TIN GIAO HÀNG\n");
        content.append("===================\n");
        content.append("Người nhận: ").append(order.getDeliveryInformation().getName()).append("\n");
        content.append("Điện thoại: ").append(order.getDeliveryInformation().getPhone()).append("\n");
        content.append("Địa chỉ: ").append(order.getDeliveryInformation().getAddress()).append("\n");
        content.append("Tỉnh/Thành phố: ").append(order.getDeliveryInformation().getProvince()).append("\n\n");
        ...
        content.append("========\n");
        content.append("Tổng tiền hàng: ").append(String.format("%,d", order.getTotalAmount())).append(" VND\n");
        content.append("Phí vận chuyển: ").append(String.format("%,d", order.getShippingFees())).append(" VND\n");
        if (order.getVAT() > 0) {
            content.append("VAT: ").append(String.format("%,d", order.getVAT())).append(" VND\n");
        }
        content.append("TỔNG CỘNG: ").append(String.format("%,d", (int) order.getTotalFees())).append(" VND\n\n");
